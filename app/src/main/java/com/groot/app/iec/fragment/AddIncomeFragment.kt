...
import com.groot.app.iec.adapter.IncomeCategoryListAdapter
import com.groot.app.iec.adapter.IncomeMoneyInListAdapter
import com.groot.app.iec.databinding.FragmentAddIncomeBinding
...
class AddIncomeFragment : BaseFragment() {
    private var binding: FragmentAddIncomeBinding? = null
    private var incomeCategoryList: ArrayList<DataItem>? = null
    private var incomeCategoryListAdapter: IncomeCategoryListAdapter? = null
    ...
    override fun onCreateView(
        ...
        if (RestConstant.UPDATE_ITEM.equals("", ignoreCase = true)) {
            ...
            binding!!.txtSelectIncomeDate.text = CurrentDate
            RestConstant.ICTextDate = CurrentDate
            IncomeDate = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).format(Date())
            IncomeDate = "$IncomeDate 00:00:00"
            RestConstant.ICDate = IncomeDate
        } else {
            IncomeDate = RestConstant.UICDate
            binding!!.txtSelectIncomeDate.text = RestConstant.UICTextDate
        ...
        binding!!.edtAddNote.addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(charSequence: CharSequence, i: Int, i1: Int, i2: Int) {
                RestConstant.ICNote = binding!!.edtAddNote.text.toString()
    ...
    private fun setDate() {
        val incomeDate =
            OnDateSetListener { view, year, monthOfYear, dayOfMonth -> // TODO Auto-generated method stub
                ...
                updateLabel(binding!!.txtSelectIncomeDate)
                val myFormat = "yyyy-MM-dd" //In which you need put here
                val sdf = SimpleDateFormat(myFormat, Locale.US)
                IncomeDate = sdf.format(myCalendar.time) + " 00:00:00"
                RestConstant.ICDate = sdf.format(myCalendar.time) + " 00:00:00"
                RestConstant.UICDate = sdf.format(myCalendar.time) + " 00:00:00"
            }
        binding!!.txtSelectIncomeDate.setOnClickListener { // TODO Auto-generated method stub
            DatePickerDialog(
                requireActivity(),
                R.style.DialogThemeBlue,
                incomeDate,
    ...
    private fun updateLabel(textView: AppCompatTextView) {
        ...
        RestConstant.ICTextDate = sdf.format(myCalendar.time)
        ...
        when (requestCode) {
            Constant.REQUEST_CODE_PICK_IMAGE -> if (resultCode == Activity.RESULT_OK) {
                ...
                for (file in list!!) {
                    ...
                    binding!!.layoutImage.visibility = View.VISIBLE
                    RestConstant.ICImageData = list!![0].path
                            ...
                            if (Objects.requireNonNull(incomeSourceResponse)?.code == 200) {
                                ...
                                if (RestConstant.UPDATE_ITEM.equals("", ignoreCase = true)) {
                                    if (incomeCategoryList != null && incomeCategoryList!!.size > 0) {
                                        ...
                                        incomeCategoryListAdapter = activity?.let {
                                            IncomeCategoryListAdapter(
                                                it, incomeCategoryList!!, this@AddIncomeFragment
                                            )
                                        }
                                        binding!!.rcvCategory.adapter = incomeCategoryListAdapter
                                    }
                                } else {
                                    for (i in incomeCategoryList!!.indices) {
                                        if (RestConstant.UICCategory.equals(
                                                incomeCategoryList!![i].name,
                                                ignoreCase = true
                                            )
                                        ) {
                                            incomeCategoryList!![i].isSelect = true
                                            break
                                        }
                                    }
                                    if (incomeCategoryList != null && incomeCategoryList!!.size > 0) {
                            ...
                            if (Objects.requireNonNull(paymentMethodResponse)?.code == 200) {
                                ...
                                if (RestConstant.UPDATE_ITEM.equals("", ignoreCase = true)) {
                                    if (moneyInList != null && moneyInList!!.size > 0) {
                                        binding!!.rcvIncomeSource.layoutManager =
                                            LinearLayoutManager(
                                                activity, LinearLayoutManager.HORIZONTAL, false
                                            )
                                        incomeMoneyInListAdapter = activity?.let {
                                            IncomeMoneyInListAdapter(
                                                it,
                                                moneyInList!!,
                                                null
                                            )
                                        }
                                        binding!!.rcvIncomeSource.adapter = incomeMoneyInListAdapter
                                    }
                                } else {
    ...
    fun getSelectedCategory(selectedCategory: String?) {
        RestConstant.ICCategory = selectedCategory!!
    }

    override fun onResume() {
        super.onResume()
        if (RestConstant.OPEN_ADD_INCOME_CATEGORY_SCREEN) {
            RestConstant.OPEN_ADD_INCOME_CATEGORY_SCREEN = false
            incomeCategoryListAPICall()
        }
    }
}